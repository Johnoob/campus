...
var moduleAjax = (function () {
    ...
    var getBtn, insertBtn, deleteBtn, dataEmail, dataPseudo, userList;
    ...
    function getSelectedIds() {
        ...
        var i, selectedBoxes, row, ids, id;

        ids = [];
        selectedBoxes = selectAll('input[type=checkbox]:checked');

        for (i = 0; i < selectedBoxes.length; i += 1) {

            row = selectedBoxes[i].parentElement.parentElement;
    ...
    function deleteViaAjax() {
        ...
        var xhr, data, ids, count;

        function handleLoad(evt) {
            /* l'appel ajax s'est déroulé sans problème */
            var result = Number(evt.target.response);
            if (count === result) {
                resetTable();
                getViaAjax();
            } else {
                showMessage('erreur de bdd, consultez le webmaster ><*', 'error');
            }
    ...
    function observer() {
        ...
        deleteBtn.addEventListener('click', deleteViaAjax);
    }

    function init() {
        getBtn = select('.action.get');
        insertBtn = select('.action.insert');
        deleteBtn = select('.action.delete');
